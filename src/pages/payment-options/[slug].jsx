...
const PaymentOption = () => {
  ...
  useEffect(()=>{
   const fetchOrder = async()=>{
    try {
      const response = await privateRequest.get(`/user/order/${router?.query?.slug }`) 
 ...
 const handleCancelOrder = async()=>{
  try {
      const response = await privateRequest.get(`/user/order/cancel/${router?.query?.slug }`) 
       router.push("/dashboard/my-order")
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-white p-4 md:p-8">
      ...
      <p className="text-sm text-gray-600 mb-6">
        Order Number: <span className="font-semibold"># {router?.query?.slug}</span>
      </p>
      ...
      {/* Cancel Button */}
      <button onClick={()=>handleCancelOrder()} className="bg-gray-300 text-gray-800 px-5 py-2 rounded hover:bg-gray-400 transition">
        CANCEL ORDER
      </button> 
      {/* Order Summary Card */}
      <div className="mt-10 lg:absolute right-10 top-20 border rounded-lg shadow-md p-6 w-full lg:w-80">
        ...
        <div className="flex justify-between font-bold text-lg text-gray-900">
          <span>Total:</span>
          <span className="text-red-600">{Math.ceil(order?.total_amount)} Tk</span>
        </div>
